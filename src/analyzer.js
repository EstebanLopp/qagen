...
async function analyzeApp(url) {
  ...
  const result = await generateTests(url, elements, bodyHTML, pageInfo, pageCategory);
...
async function generateTests(url, elements, bodyHTML, pageInfo, pageCategory) {
...
test.describe('Nombre descriptivo', () => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // Error de red, rate limit, clave inválida, etc.
      // Se loguea el intento y se reintenta si quedan intentos
      console.log(`⚠️  Intento ${attempt}/${maxRetries}: error al llamar OpenAI — ${apiError.message}`);
      if (attempt === maxRetries) return null;
      continue;
    }

    // La limpieza de markdown se hace aquí, una sola vez,
    // antes de validar la sintaxis
...
function saveTests(code, url) {
  ...
  // Toda la limpieza y corrección ocurre aquí, una sola vez,
  // antes de escribir el archivo al disco
  let clean = code
    .replace(/```javascript\n?/g, '')
    .replace(/```\n?/g, '');
